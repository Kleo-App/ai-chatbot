          ...
          <div className={`bg-white rounded-lg border border-foreground/20 shadow-sm mx-auto ${deviceType === 'mobile' ? 'w-[375px]' : 'w-[552px]'}`}>
            ...
            <div className="flex flex-row justify-between px-4 pt-3 pb-2">
              <div className="flex flex-row gap-2">
                <div className="relative flex size-8 shrink-0 overflow-hidden h-12 w-12 rounded-full">
                  {userProfile?.profileImage ? (
                    <img 
                      className="size-full aspect-square h-full w-full" 
              ...
              {showDeviceToggle && (
                <div className="flex items-center gap-1">
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className={`h-6 w-6 p-0 ${deviceType === 'mobile' ? 'bg-primary text-primary-foreground' : ''}`}
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className={`h-6 w-6 p-0 ${deviceType === 'desktop' ? 'bg-primary text-primary-foreground' : ''}`}
                    onClick={() => onToggleDevice?.()}
                  >
                    <MonitorIcon size={12} />
                  </Button>
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className={`h-6 w-6 p-0 ${isCollapsed ? 'bg-primary text-primary-foreground' : ''}`}
                ...
                <div className="flex flex-row">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" className="h-4 w-4">
                    <g>
                      ...
                      <circle cx="8" cy="8" r="7" fill="#df704d"></circle>
                      <path d="M7.71 5A2.64 2.64 0 004 8.75l4 4 4-4A2.64 2.64 0 0012 5a2.61 2.61 0 00-1.85-.77h0A2.57 2.57 0 008.3 5l-.3.3z" fill="#fff3f0" stroke="#77280c" fillRule="evenodd"></path>
                    </g>
                  </svg>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" className="-ml-1 h-4 w-4">
                    <g>
                      <path d="M8 0a8 8 0 018 8 8 8 0 01-8 8 8 8 0 01-8-8 8 8 0 018-8z" fill="none"></path>
            ...
            <div className="border-t border-foreground/10 flex flex-row justify-between px-4 py-2">
              <div className={`flex w-full flex-row justify-evenly ${deviceType === 'mobile' ? 'gap-0' : 'gap-2'}`}>
                <Button variant="ghost" className={`inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground flex-1 ${deviceType === 'mobile' ? 'h-8 px-1 py-1' : 'h-10 px-4 py-2'}`}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className={`${deviceType === 'mobile' ? 'mr-0.5 h-4 w-4' : 'mr-1 h-6 w-6'} text-black/60`} fill="currentColor">
                ...
                <Button variant="ghost" className={`inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground flex-1 ${deviceType === 'mobile' ? 'h-8 px-1 py-1' : 'h-10 px-4 py-2'}`}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className={`${deviceType === 'mobile' ? 'mr-0.5 h-4 w-4' : 'mr-1 h-6 w-6'} text-black/60`}>
